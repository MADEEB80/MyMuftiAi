...
export default function CategoriesPage() {
        ...
        if (!categoriesSnapshot.empty) {
          ...
          categoriesSnapshot.forEach((doc) => {
            ...
            fetchedCategories.push({
              id: doc.id,
              name: data.name || t("categories.unknown"),
          ...
          if (!questionsSnapshot.empty) {
            ...
            categoryMap.forEach((count, value) => {
              defaultCategories.push({
                ...
                value: value,
                description: t("categories.defaultDescription", { value }),
                questionCount: count,
              })
            })
      ...
      try {
        ...
        if (error.message && error.message.includes("index")) {
          ...
          if (urlMatch) {
            ...
            setIndexError(t("categories.indexError"))
          }
        }
  ...
  return (
    <div className="container py-10" dir={isRTL ? "rtl" : "ltr"}>
      <h1 className="mb-6 text-3xl font-bold">{t("categories.title")}</h1>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
        ...
        <div className="md:col-span-3">
          <Card>
            <CardHeader>
              <CardTitle>{selectedCategoryData?.name || t("categories.questions")}</CardTitle>
            ...
            <CardContent>
              {indexError && (
                <Alert variant="destructive" className="mb-4">
                  ...
                  <AlertDescription className="flex items-center justify-between">
                    <span>{t("categories.indexErrorDescription")}</span>
                    {indexError.startsWith("https://") ? (
                      <a
                        href={indexError}
                        target="_blank"
                      ...
                      >
                        {t("categories.createIndex")} <ExternalLink className="ml-1 h-3 w-3" />
                      </a>
                    ) : null}
                  </AlertDescription>
                </Alert>
              ...
              {loadingQuestions ? (
                ...
                <div className="flex h-40 items-center justify-center">
                  <p className="text-muted-foreground">{t("categories.noQuestions")}</p>
                </div>
              ) : (
            ...
            <CardFooter>
              <div className="flex w-full items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  {t("categories.showingQuestions", { count: questions.length })}
                </p>
                <Link href="/dashboard/ask">
                  <Button>{t("categories.askQuestion")}</Button>
                </Link>
              </div>
            </CardFooter>
          </Card>
